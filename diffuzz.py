...
class Diffuzz():
    ...
    def sample(self, model, model_inputs, shape, mask=None, t_start=1.0, t_end=0.0, timesteps=20, x_init=None, cfg=3.0,
               unconditional_inputs=None, sampler='ddpm', half=False, targetImage=None):
        ...
        for i in range(0, timesteps):
            ...
            #if(i<66 and targetImage is not None):
            #x = self.undiffuse(targetImage, r_range[i], r_range[i + 1], pred_noise, sampler=sampler)
            #else:
