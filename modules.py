...
def sample(model, model_inputs, latent_shape, unconditional_inputs=None, init_x=None, steps=12, renoise_steps=None, temperature = (0.7, 0.3), cfg=(8.0, 8.0), mode = 'multinomial', t_start=1.0, t_end=0.0, sampling_conditional_steps=None, sampling_quant_steps=None, attn_weights=None, targetImage = None): # 'quant', 'multinomial', 'argmax'
    ...
    for i, tv in enumerate(t_list[:steps]):
        ...
        if i < renoise_steps:
            ...
            if(i<5 and targetImage is not None):
                sampled = model.add_noise(targetImage, t_next, random_x=init_noise)[0]
            else:
